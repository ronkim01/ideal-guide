...
4. **GitHub Actions**의 실행 버튼을 누르면 이 사이클이 클라우드에서 돌아가고 결과가 자동 커밋됩니다. (원하면 매일 자동 실행으로 전환 가능)
...
GitHub → `Actions` 탭 → `AI Company Run` → `Run workflow` (목표 입력 가능, 비워두면 대표실이 스스로 결정)

기본은 **수동 실행 모드**라 실행할 때만 비용이 발생합니다.
매일 아침 9시(KST) 자동 실행으로 바꾸려면 `.github/workflows/ai-company.yml`에서 `schedule` 주석을 해제하세요.
...
코인 매매를 혼자 기록하고 승률·손익비·기대값을 자동 계산하는 1인용 웹 앱입니다.
폰·PC 어디서 열어도 같은 데이터를 보며, 기록은 Supabase에 저장됩니다(내 계정만 접근).
매월 `Actions` 탭의 **매매일지 월간 복기** 버튼을 누르면 한 달치 기록을 AI가 복기해
앱의 [복기] 탭에 띄워줍니다.

- 설치·사용법: [`journal/README.md`](journal/README.md)
- 설계 배경: [`docs/매매일지-기획서.md`](docs/매매일지-기획서.md)
